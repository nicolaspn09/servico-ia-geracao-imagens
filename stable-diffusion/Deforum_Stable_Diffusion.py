...
def getRotationMatrixManual(rotation_angles):
    ...
    COMPANY_NAME       = rotation_angles[1] # around y
    ...
    # Y rotation
    RCOMPANY_NAME        = np.eye(4,4)
    sg            = np.sin(COMPANY_NAME)
    cg            = np.cos(COMPANY_NAME)
    RCOMPANY_NAME[0,0]   = cg
    RCOMPANY_NAME[2,2]   = RCOMPANY_NAME[0,0]
    RCOMPANY_NAME[0,2]   = sg
    RCOMPANY_NAME[2,0]   = -sg
    ...
    R           = reduce(lambda x,y : np.matmul(x,y), [Rphi, RCOMPANY_NAME, Rtheta]) 
...
def warpMatrix(W, H, theta, phi, COMPANY_NAME, scale, fV):
    ...
    R = getRotationMatrixManual([phi, COMPANY_NAME, theta])
...
def anim_frame_warp_2d(prev_img_cv2, args, anim_args, keys, frame_idx):
    ...
    if anim_args.flip_2d_perspective:
        ...
        perspective_flip_COMPANY_NAME = keys.perspective_flip_COMPANY_NAME[frame_idx]
        perspective_flip_fv = keys.perspective_flip_fv_series[frame_idx]
        M,sl = warpMatrix(args.W, args.H, perspective_flip_theta, perspective_flip_phi, perspective_flip_COMPANY_NAME, 1., perspective_flip_fv);
...
def DeforumAnimArgs():
    ...
    perspective_flip_COMPANY_NAME = "0:(0)"#@param {type:"string"}
...
class DeformAnimKeys():
    def __init__(self, anim_args):
        ...
        self.perspective_flip_COMPANY_NAME = get_inbetweens(parse_key_frames(anim_args.perspective_flip_COMPANY_NAME), anim_args.max_frames)
